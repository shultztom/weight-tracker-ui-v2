...
const dateRules = [
  v => !!v || 'Required',
  v => new Date(v) > new Date() || 'Goal date must be in the future',
];
...
const loadGoal = async () => {
  try {
    const response = await axios({
      url: `${URL}/goals/${userStore.getUser}`,
    ...
    if (goals && goals.length > 0) {
      ...
      goalWeightLbs.value = (latest.weight * 2.20462).toFixed(1);
...
const saveGoal = async () => {
  saving.value = true;
  const payload = {
    weight: goalWeightLbs.value / 2.20462,
  ...
  try {
    if (editMode.value) {
      await axios({
        method: 'PUT',
        url: `${URL}/goals/${userStore.getUser}/${goalId.value}`,
        headers: {
